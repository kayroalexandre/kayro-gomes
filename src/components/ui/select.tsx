...
const SelectTrigger: React.FC<React.ComponentProps<typeof SelectPrimitive.Trigger>> = ({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        "border-input data-[placeholder]:text-muted-foreground aria-invalid:border-destructive ring-ring/10 dark:ring-ring/20 dark:outline-ring/40 outline-ring/50 [&_svg:not([class*='text-'])]:text-muted-foreground flex h-[var(--control-height-sm)] w-full items-center justify-between rounded-md border bg-transparent px-[var(--control-padding-x-compact)] py-[var(--control-padding-y)] text-body-sm shadow-xs transition-[color,box-shadow] focus-visible:ring-4 focus-visible:outline-1 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:focus-visible:ring-0 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&>span]:line-clamp-1",
        className,
      )}
      {...props}
    >
...
const SelectLabel: React.FC<React.ComponentProps<typeof SelectPrimitive.Label>> = ({
  ...
  return (
    <SelectPrimitive.Label
      ...
      className={cn('px-2 py-1.5 text-body-sm font-semibold', className)}
      {...props}
    />
  )
}
...
const SelectItem: React.FC<React.ComponentProps<typeof SelectPrimitive.Item>> = ({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "focus:bg-accent focus:text-accent-foreground [&_svg:not([class*='text-'])]:text-muted-foreground relative flex w-full cursor-default items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-body-sm outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className,
      )}
      {...props}
    >
